...
def main():
    ...
    result = mysql_handler.query_data(
        "SELECT file_path FROM `local`.`file` WHERE `c_name` IS NULL AND `c_group` IS NULL  AND `content` not LIKE '<x:xmpmeta %' AND `content` not LIKE 's = {%';")
    # 遍历查询结果
    for entry in result:
        file_path = entry[0]
        ...
        # 构建目标文件夹路径
        target_folder = r'D:\Users\Documents\预设\回收站老预设\无效预设'
    ...
    mysql_handler.execute_sql(
        "DELETE FROM `local`.`file` WHERE `c_name` IS NULL AND `c_group` IS NULL  AND `content` not LIKE '<x:xmpmeta %' AND `content` not LIKE 's = {%';")
    sys.exit()
